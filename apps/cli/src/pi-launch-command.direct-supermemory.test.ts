...
describe("runPiLaunch direct Supermemory dashboard config", () => {
  test("fails closed for invalid dashboard container tags without injecting memory", () => {
  ...
  test("validates direct dashboard Supermemory config like Deck config and rejects secret fields", () => {
    const projectRoot = createTempDir();
    const piMcpConfigPath = join(projectRoot, "home", ".pi", "agent", "mcp.json");
    try {
      writePiMcpConfig(piMcpConfigPath);
  ...
  test("validates direct dashboard Supermemory config like Deck config and rejects extra fields", () => {
    const projectRoot = createTempDir();
    const piMcpConfigPath = join(projectRoot, "home", ".pi", "agent", "mcp.json");
    try {
      writePiMcpConfig(piMcpConfigPath);
