...
## Live Adaptive Memory acceptance repairs

Cross-session acceptance exposed that OpenCode advisory delivery used `experimental.chat.messages.transform` to add a raw `system` message that OpenCode drops during model-message conversion. The plugin now keeps one bounded pending advisory per native session and consumes it once through `experimental.chat.system.transform`. A pre-request generation guard makes the latest invocation win even when an older recall finishes later, and session deletion invalidates pending state.

The same acceptance pass made explicit recall return an actionable no-match result when provider operations succeed without renderable context, and removes all inherited runner-memory/Codex bridge namespace values before applying current-session overlays. Nested, degraded, and static-compatible launches therefore cannot inherit parent bridge authority.

A Spanish Orion/Nebula acceptance run then exposed that Capture Policy recognized durable categories only in English. Minimal Spanish durable-decision and explicit-remember patterns now admit the tested architectural decision while preserving secret/artifact/log/trivial rejection precedence. OpenCode role recall is also restricted to actual task/delegate requests with recognized Deck roles, eliminating profile-recall noise from ordinary tools.

The final candidate passed 154 focused capture/runner/runtime/plugin tests, TypeScript checking, compiled standalone runtime verification, generated-asset parity, and diff checking. A fresh clean project scope captured the original Spanish Session A and a separate Session B returned Orion and Nebula Boundary without Context Mode or local repository inspection. In the intentionally polluted Esprit scope, focused explicit recall also found Orion/Nebula; broad ranking favored older near-identical canary memories, which is provider ranking over valid same-project data rather than a capture or isolation defect. Final independent Quality returned GO with no blockers. No release, publication, commit, push, or archive occurred.
