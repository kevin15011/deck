...
class OpenCodeRunnerAdapterImpl {
  ...
  /**
   * OpenCode's adapter has no host API that can request and observe a native
   * skill load. Keep the adapter boundary explicit so callers cannot mistake a
   * resolved filesystem observation for native execution.
   */
  readonly skillLoading: SkillNativeLoadPortV1 = {
    schema: "skill-native-load-port-v1",
    prepare: async () => ({ outcome: "unsupported" }),
