...
class OpenCodeRunnerAdapterImpl implements RunnerAdapter {
  ...
  async applyDeveloperTeamInstall(input: DeveloperTeamApplyInput): Promise<DeveloperTeamApplyResult> {
    ...
    const nativePlan: OpenCodeDeveloperTeamInstallPlan = {
      ...
      promptGenerationPlan: this.#lastNativePlan?.promptGenerationPlan ?? [],
      commandGenerationPlan: this.#lastNativePlan?.commandGenerationPlan ?? [],
      mermaidPluginStatus: this.#lastNativePlan?.mermaidPluginStatus ?? "missing",
      capabilityInstructions: this.#lastNativePlan?.capabilityInstructions,
      memoryBundle: this.#lastNativePlan?.memoryBundle,
      personality: this.#lastNativePlan?.personality,
    };
