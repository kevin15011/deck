...
describe("Authorization Card and Self-Rejection Instruction", () => {
  test("AGENT_BODY contains Authorization Card section header", () => {
    expect(APPLY_BACKEND_AGENT_BODY).toContain("## Authorization Card");
  });

  test("AGENT_BODY contains the orchestrator injection placeholder comment", () => {
    expect(APPLY_BACKEND_AGENT_BODY).toContain(
  ...
  test("AGENT_BODY contains Self-Rejection Instruction section", () => {
    expect(APPLY_BACKEND_AGENT_BODY).toContain("## Self-Rejection Instruction");
  });

  test("AGENT_BODY contains refusal phrase for missing authorization", () => {
    expect(APPLY_BACKEND_AGENT_BODY).toContain("refuse to perform any file modifications");
    expect(APPLY_BACKEND_AGENT_BODY).toContain("Report `blocked` status");
  });
