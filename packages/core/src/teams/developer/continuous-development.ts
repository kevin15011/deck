...
- Treat the Orchestrator's continuation authorization as an ordinary modifying batch. Do not require a new proposal, task artifact, phase restart, or repeated user confirmation solely because the instruction is incremental.
...
- Exercise the changed behavior through its relevant interface after each delta. Return the observed result, changed targets, invalidated evidence, and focused Apply-local checks so the Orchestrator can present a useful result quickly.
