...
describe("EG2-R2 public trust-boundary RED matrix", () => {
  ...
  // B-B7-PUBLIC-EXPORT-ORACLE-REMAINS-SUBSET-v1: exact complete Batch B V1 surface.
  // Uses literal 57-key sorted equality to catch both missing AND extra exports.
  test("B-B7 exports exactly 57 public functions at package root", () => {
    const expected = ["DEFAULT_BUDGET_CONFIG","DEFAULT_DISCOVERY_CONFIG","DEFAULT_ENFORCEMENT_CONFIG","DEFAULT_LOOP_BREAKER_CONFIG","DEFAULT_ORCHESTRATOR_PERSONALITY","DEFAULT_PIPELINE_CONFIG","DEFAULT_RECOVERY_CONFIG","DEFAULT_RISK_THRESHOLDS","DEFAULT_ROUTER_CONFIG","DEFAULT_RUNNER_PIPELINE_CONFIG","DEFAULT_SCORER_CONFIG","ORCHESTRATOR_PERSONALITIES","adaptRepairIncidentToFailureManifestV1","applyEnforcement","attemptResume","buildApplyBatchContractV1","buildAuthorizationReferenceV1","buildCausalContextV1","buildExecutionDecisionV1","buildFailureManifestV1","buildInvocationAuthorizationClaimsV1","buildLaneDecisionV1","buildRegistryIntentV1","buildStagedVerificationStateV1","checkBudget","checkLoopCondition","classifyFailure","classifyTransportFailure","computeFailureDeltaV1","computeRiskScore","createExecutionDossierV1","createProjectDiscoveryAdapter","parseApplyBatchContractV1","parseAuthorizationReferenceV1","parseCausalContextV1","parseExecutionDecisionV1","parseExecutionDossierV1","parseFailureDeltaV1","parseFailureManifestV1","parseInvocationAuthorizationClaimsV1","parseLaneDecisionV1","parseRegistryIntentV1","parseRepairIncidentYAML","parseStagedVerificationStateV1","resolveEnforcementMode","reviseExecutionDossierV1","routeQuality","runOrchestratorPipeline","runRunnerPipeline","submitStateUpdate","validateAdapterCapabilities","validateArtifactForResume","validatePhaseOutcome","validateRiskResult","validateSelfAudit","validateStateUpdate","validateTransportContext"] as const;
    expect(Object.keys(publicApi).sort()).toEqual([...expected].sort());
  });
