...
export interface RegistryIntentV1 { schema: "registry-intent-v1"; intentId: `registry-intent:v1:${string}`; digest: Sha256Digest; idempotencyKey: Sha256Digest; changeId: string; batchId?: BatchId; batchDigest?: Sha256Digest; base: { stateDigest: Sha256Digest; eventsDigest: Sha256Digest }; phase: string; status: string; artifact: { kind: string; path: string; digest?: Sha256Digest }; provenance: { agent: string; model: string; timestamp: string; note?: string }; event: { name: string; actor: string; timestamp: string; notes: readonly string[] }; decisionDigest?: Sha256Digest }
...
export function parseRegistryIntentV1(value: unknown): RegistryIntentV1 {
  assertExactKeys(value, ["schema", "intentId", "digest", "idempotencyKey", "changeId", "batchId", "batchDigest", "base", "phase", "status", "artifact", "provenance", "event", "decisionDigest"], "registry intent fields");
