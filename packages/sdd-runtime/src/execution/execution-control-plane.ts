...
export type ExecutionRoleScheduleResultV1 =
  ...
  | {
      readonly code: "not-required" | "invalid-evidence" | "verification-evidence-required" | "lane-floor-violation";
      readonly rationaleCodes: readonly string[];
...
export async function commitExecutionRegistryIntentsV1(
  ...
  const outcomes: Array<ExecutionRegistryCommitResultV1["outcomes"][number]> = [];
  for (const intent of dossier.registryIntents) {
    let outcome: Awaited<ReturnType<ExecutionRegistryCoordinatorPortV1["commit"]>>;
    try { outcome = await coordinator.commit(intent); }
    catch { outcome = { code: "registry-recovery-required", intentId: intent.intentId }; }
    const code = outcome.intentId !== undefined && outcome.intentId !== intent.intentId ||
      (outcome.code === "committed" || outcome.code === "replayed") && outcome.intentId === undefined
      ? "invalid-evidence"
      : outcome.code;
    outcomes.push(deepFreeze({ intentId: intent.intentId, code }));
    if (code !== "committed" && code !== "replayed" && code !== "distributed-compatible") {
...
export function parseExecutionRoleInvocationV1(value: unknown): ExecutionRoleInvocationV1 {
  assertExactKeys(value, [
    ...
    "requiresFreshAgent", "rationaleCodes", "causalContext", "dependencies",
  ], "role invocation");
  ...
  assertExactKeys(value.dependencies, ["batchDigest", "dossierDigest", "decisionDigest", "verificationDigest"], "role invocation.dependencies");
  const dependencies = {
  ...
  const payload = {
    ...
    checkIds: stringArray(value.checkIds, "role invocation.checkIds", true),
...
export function scheduleExecutionRoleInvocationV1(
  ...
  try {
    ...
    if (role === "verify") {
      stage = safePlan.decision.action === "advance_verification"
      ...
      if (safePlan.decision.action !== "complete" || !freshness.freshReviewRequired) {
        return deepFreeze({ code: "not-required", rationaleCodes: ["REVIEW_NOT_REQUIRED"] });
      }
      checkIds = stringArray(input.reviewCheckIds, "role scheduling.reviewCheckIds", false);
      if (checkIds.length === 0) return deepFreeze({ code: "invalid-evidence", rationaleCodes: ["REVIEW_CHECKS_REQUIRED"] });
...
export function consumeExecutionRoleResultV1(
  ...
  try {
    ...
    assertExactKeys(resultValue, [
      "schema", "invocationId", "role", "agentInstanceId", "batchId", "stage", "status", "evidence",
      "omission", "provenance", "dependencies", "registryIntents",
    ], "role result");
    ...
    assertExactKeys(resultValue.dependencies, ["batchDigest", "dossierDigest", "decisionDigest", "verificationDigest"], "role result.dependencies");
    ...
    const normalizedResult = deepFreeze({
      ...
      registryIntents,
    }) as ExecutionRoleResultEnvelopeV1;
    ...
    if (role === "verify") {
      ...
      const acceptance = validateVerificationAcceptanceV1(safePlan.dossier.verification, effectiveVerificationPolicy);
      if (acceptance.code !== "complete") {
        const code = acceptance.code === "advanced" ? "invalid-evidence" : acceptance.code;
